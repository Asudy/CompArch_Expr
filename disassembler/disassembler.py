...
[Change Log]
2020-12-14      - Add an option for "R0" ~ "R32" register names.
2021-01-13      - Add sra, sllv, srlv, srav to the R-type instruction list.
                - Skip empty instructions (00000000) instead of translating them into "sll $zero $zero".
'''
import sys
...
## Added sra, sllv, srlv, srav instruction in Computer Architecture Lab 5.
RTypeFunct = {  0x20: "add ${2:} ${0:} ${1:}",  0x21: "addu ${2:} ${0:} ${1:}", 0x22: "sub ${2:} ${0:} ${1:}", \
                ...
                0x8:  "jr ${0:}",               0xC:  "syscall",                0x3:  "sra ${2:} ${1:} {3:}",  \
                0x4:  "sllv ${2:} ${1:} ${0:}", 0x6:  "srlv ${2:} ${1:} ${0:}", 0x7:  "srav ${2:} ${1:} ${0:}" }
...
unsignedExtITypeName = ["andi", "ori", "xori", "lui"]
...
for i in range( len(lines) ):   # Suppose each line is a base 16 instruction
    ...
    try:
        ...
        # Output 8'b0 if the instruction is empty
        if inst_bin == '0' * 32: 
            outFile.write('0' * 8 + "\n")
            continue
        ...
        if instOpCode == 0 :
            ...
            # I-Type instruction
                ...
                if OpCode[instOpCode].split()[0] not in unsignedExtITypeName:
